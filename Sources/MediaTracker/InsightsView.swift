...
struct InsightsView: View {
        ...
        VStack(alignment: .leading, spacing: 32) {
            ...
            VStack(alignment: .leading, spacing: 20) {
                metricSimple(
                    ...
                    value: formatWatchTimeSimple(minutes: stats.totalWatchTimeMinutes))
                metricSimple(
    ...
    private func hallOfFameSection(stats: LibraryStats) -> some View {
        VStack(alignment: .leading, spacing: 80) {
            hallOfFameRow(
                title: "ACTORS HALL OF FAME", people: stats.topRatedActors, color: .orange)
            hallOfFameRow(
                title: "CREATORS HALL OF FAME", people: stats.topRatedCreators, color: .green)
        }
    }
    ...
    private func productionDeckSection(stats: LibraryStats) -> some View {
        VStack(alignment: .leading, spacing: 60) {
            ...
            VStack(alignment: .leading, spacing: 50) {
                ...
                horizontalAffinityList(
                    title: "Highest Rated Genres", data: stats.topRatedGenres, icon: "tag.fill",
                    color: .blue)
            }
        }
    }
    ...
    private func masterySection(stats: LibraryStats) -> some View {
        VStack(alignment: .leading, spacing: 40) {
            ...
            HStack(spacing: 30) {
                masteryCard(
                    title: "Movies", current: stats.completedMovies, total: stats.totalMovies,
                    color: .blue)
                masteryCard(
                    title: "TV Shows", current: stats.completedTVShows, total: stats.totalTVShows,
                    color: .purple)
                masteryCard(
                    label: "EPISODES", value: "\(stats.totalEpisodesWatched)", subtitle: "WATCHED",
                    color: .orange)
            }
        }
    }
    ...
    @ViewBuilder
    private func horizontalAffinityList(
        title: String, data: [(String, Double)], icon: String, color: Color
    ) -> some View {
        if !data.isEmpty {
            VStack(alignment: .leading, spacing: 20) {
                HStack(spacing: 12) {
                    Image(systemName: icon).foregroundStyle(color).font(
        ...
        VStack(alignment: .leading, spacing: 16) {
            ZStack {
                if let urlString = person.profileURL, let url = URL(string: urlString) {
                    ...
                    Color.secondary.opacity(0.1)
                        .overlay {
                            Image(systemName: "person.fill").font(.largeTitle).foregroundStyle(
                                .secondary)
                        }
                }
            }
    ...
    @ViewBuilder
    private func masteryCard(
        title: String? = nil, label: String? = nil, value: String? = nil, subtitle: String? = nil,
        current: Int = 0, total: Int = 0, color: Color
    ) -> some View {
        VStack(alignment: .leading, spacing: 20) {
            if let title = title {
                Text(title.uppercased()).font(.system(size: 10, weight: .black)).kerning(2)
                    .foregroundStyle(.secondary)
                HStack(alignment: .firstTextBaseline, spacing: 12) {
                    Text("\(current)").font(.system(size: 42, weight: .black, design: .rounded))
                    Text("/ \(total)").font(.system(size: 18, weight: .bold)).foregroundStyle(
                        .secondary)
                }
                ProgressView(value: total > 0 ? Double(current) / Double(total) : 0)
                    .tint(color)
            } else {
                Text(label ?? "").font(.system(size: 10, weight: .black)).kerning(2)
                    .foregroundStyle(.secondary)
                Text(value ?? "").font(.system(size: 42, weight: .black, design: .rounded))
                    .foregroundStyle(color)
                Text(subtitle ?? "").font(.system(size: 10, weight: .bold)).foregroundStyle(
                    .secondary)
            }
        }
        .padding(32)
    ...
    private var loadingView: some View {
        VStack(spacing: 30) {
            ...
            Text("EXTRACTING CINEMA DNA").font(.system(size: 12, weight: .black)).kerning(2)
                .foregroundStyle(.secondary)
        }
