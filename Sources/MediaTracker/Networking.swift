...
class APIClient {
    ...
    var tmdbApiKey: String = UserDefaults.standard.string(forKey: "tmdb_api_key") ?? ""
    var googleBooksApiKey: String = UserDefaults.standard.string(forKey: "google_books_api_key") ?? ""
    
    private func tmdbURL(path: String, queryItems: [URLQueryItem] = []) -> URL? {
        var components = URLComponents(string: "https://api.themoviedb.org/3\(path)")
        var items = [URLQueryItem(name: "api_key", value: tmdbApiKey)]
    ...
    func searchMovies(query: String) async throws -> [MovieSearchResult] {
        ...
        guard let url = tmdbURL(path: "/search/movie", queryItems: [URLQueryItem(name: "query", value: query)]) else { throw URLError(.badURL) }
    ...
    func searchTVShows(query: String) async throws -> [TVSearchResult] {
        guard !tmdbApiKey.isEmpty else { return [] }
        guard let url = tmdbURL(path: "/search/tv", queryItems: [URLQueryItem(name: "query", value: query)]) else { throw URLError(.badURL) }
        
        let (data, _) = try await URLSession.shared.data(from: url)
        let response = try decoder.decode(TMDBTVResponse.self, from: data)
        return response.results.map { 
    ...
    func searchBooks(query: String) async throws -> [BookSearchResult] {
        ...
        if !googleBooksApiKey.isEmpty {
            items.append(URLQueryItem(name: "key", value: googleBooksApiKey))
        }
        components?.queryItems = items
    ...
    func fetchTrendingMovies() async throws -> [MovieSearchResult] {
        guard !tmdbApiKey.isEmpty else { return [] }
        guard let url = tmdbURL(path: "/trending/movie/day") else { throw URLError(.badURL) }
        let (data, _) = try await URLSession.shared.data(from: url)
        let response = try decoder.decode(TMDBResponse.self, from: data)
        return response.results.prefix(10).map { 
            MovieSearchResult(id: String($0.id), title: $0.title, overview: $0.overview, posterURL: "https://image.tmdb.org/t/p/w500\($0.poster_path ?? "")", releaseDate: $0.release_date)
        }
    }
    ...
    func fetchTrendingTVShows() async throws -> [TVSearchResult] {
        guard !tmdbApiKey.isEmpty else { return [] }
        guard let url = tmdbURL(path: "/trending/tv/day") else { throw URLError(.badURL) }
        let (data, _) = try await URLSession.shared.data(from: url)
        let response = try decoder.decode(TMDBTVResponse.self, from: data)
        return response.results.prefix(10).map { 
            TVSearchResult(id: String($0.id), title: $0.name, overview: $0.overview, posterURL: "https://image.tmdb.org/t/p/w500\($0.poster_path ?? "")", releaseDate: $0.first_air_date)
    ...
    func fetchMovieDetails(tmdbID: Int) async throws -> (runtime: Int?, genres: [String], voteAverage: Double?, releaseDate: String?) {
        guard !tmdbApiKey.isEmpty else { return (nil, [], nil, nil) }
        guard let url = tmdbURL(path: "/movie/\(tmdbID)") else { throw URLError(.badURL) }
        let (data, _) = try await URLSession.shared.data(from: url)
    ...
    func fetchSeasonDetails(tmdbID: Int, seasonNumber: Int) async throws -> [TVEpisodeResult] {
        guard !tmdbApiKey.isEmpty else { return [] }
        guard let url = tmdbURL(path: "/tv/\(tmdbID)/season/\(seasonNumber)") else { throw URLError(.badURL) }
        let (data, _) = try await URLSession.shared.data(from: url)
        let response = try decoder.decode(TMDBSeasonResponse.self, from: data)
    ...
    func fetchTVMazeSchedule(tvMazeID: Int) async throws -> (episode: TVMazeEpisode?, timezone: String?, serviceName: String?) {
        ...
        let (data, _) = try await URLSession.shared.data(from: url)
        let response = try decoder.decode(TVMazeResponse.self, from: data)
        let serviceName = response.webChannel?.name ?? response.network?.name
        return (response._embedded?.nextepisode, response.timezone, serviceName)
    }
}
